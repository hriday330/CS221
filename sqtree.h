...
private:
  ...
  PNG im; //image that the tree represents. Will be modified to be the blocky/compressed image once render is called 
  ...
  /**
   * Private helper function for the copy function. Recursively copies 
   * the tree from the other node given. 
   * @param other Contains the other node used to recursively copy the tree.
  */

  Node* copyHelper(Node *other);

  /**
   * Private helper function for the render function. Recursively renders the tree, 
   * modifiying the image member of sqtree when reaching leaf nodes. 
   * The leaf nodes are rendered by using the average pixel stored. 
   * @param subtree Contains the node used to recursively render the tree.
  */
  void renderHelper(Node*subtree);
  /**
   * Private helper function for the clear function. Recursively copies 
   * the tree from the other node given and frees memory. 
   * @param subRoot Contains the other node used to recursively clear the tree. 
  */
  void clearHelper(Node *&subRoot);

  /**
   * Private helper function for the buildtree function. Iterates through the given 
   * subrectangle and finds the partition (x,y) within the dimensions minimising the 
   * maximum variability of the subrectangles. 
   * @param s the stats object used to calculate the variance for the subrectangles
   * @param ul the top-left corner of current rectangle
   * @param w the width of the current rectangle
   * @param h the height of the current rectangle
  */
  pair<int, int> findBestPartition(stats&s, pair<int,int> &ul, int w, int h);
